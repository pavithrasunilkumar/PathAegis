  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await register({
        ...
        email: form.email.trim() || `${form.username.trim()}@pathaegis.local`,
    ...
    <div style={{
      ...
      <div className="glass fade-in" style={{
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom: 16 }}>
            <label style={{ display: 'block', fontSize: 11, fontFamily: 'var(--font-mono)', color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: '0.12em', marginBottom: 6 }}>
              Email <span style={{ color: 'var(--text-muted)', fontSize: 10 }}>(optional)</span>
            </label>
            <input
              className="input"
              name="email"
          ...
          {success && (
            <div style={{ marginBottom: 16, padding: '10px 14px', background: 'rgba(34,211,238,0.1)', border: '1px solid rgba(34,211,238,0.35)', borderRadius: 6, color: 'var(--neon-cyan)', fontSize: 13 }}>
              {success}
            </div>
          )}
